#!/usr/bin/env python3
"""
Extract full-question images from katalog3.pdf.

Katalog3 has exactly 5 questions per page. Each question band is bounded by:
  - Top: blue number frame (cluster of blue rectangles around the white digit)
  - Bottom: red dotted separator line before the next question, or page footer
    for the 5th question on the page
"""
...
def extract_all_images(pdf_path: Path, output_dir: Path) -> dict[int, str]:
    ...
    for page_index, page in enumerate(doc):
        ...
        if len(bounds) not in (0, 5):
            numbers = sorted(bounds)
            print(f"WARN page {page_index + 1}: expected 5 questions, got {len(bounds)} ({numbers})")
