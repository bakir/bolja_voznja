#!/usr/bin/env python3
"""
Extract answers and question images from katalog2.pdf.

Same geometry approach as katalog1, with wider x tolerances for the
right-shifted option column. Writes {answers, option_count, image} per question.
"""
...
KATALOG2_OPTION_MARKER_X_MAX = 95.0
KATALOG2_LINE_X_MAX = 95.0
IMAGE_X_MIN = 350.0
DISTANCE_WARN_THRESHOLD = 3.0


def configure_katalog2_geometry() -> None:
    ea.OPTION_MARKER_X_MAX = KATALOG2_OPTION_MARKER_X_MAX
    ea.LINE_X_MAX = KATALOG2_LINE_X_MAX
...
def extract_katalog2(
    pdf_path: Path,
    images_dir: Path,
    questionpics_dir: Path,
) -> dict[int, dict]:
    configure_katalog2_geometry()
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Extract answers and images from katalog2.pdf.",
    )
    parser.add_argument("pdf", nargs="?", default="katalog2.pdf")
    parser.add_argument(
        "-o",
        "--output",
        default="katalog2_answers.json",
        help="JSON output path (default: katalog2_answers.json)",
    )
    parser.add_argument(
        "--images-dir",
        default="katalog2_images",
        help="Directory for embedded diagram images (default: katalog2_images)",
    )
    parser.add_argument(
        "--questionpics-dir",
        default="katalog2_questionpics",
        help="Directory for full question PDF cutouts (default: katalog2_questionpics)",
    )
    ...
    detail = extract_katalog2(pdf_path, images_dir, questionpics_dir)
    ...
    if missing_images:
        print(f"\n# missing images for questions: {missing_images}")
